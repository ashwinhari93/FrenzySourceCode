...
var cropForSearch = angular.module('ngImgCropForSearch', []);
/*var minHeightAllowed = 1000;
var minWidthAllowed = 1192;
*/
var minHeightAllowed = 700;
var minWidthAllowed = 1170;
var dimensionValidFlag = true;
var checkdimensionValidity = true;/*mark false if dont want to check dimension validity*/

cropForSearch.factory('cropAreaCircleForSearch', ['cropAreaForSearch', function(cropAreaForSearch) {
  var CropAreaCircle = function() {
    cropAreaForSearch.apply(this, arguments);
  ...
  CropAreaCircle.prototype = new cropAreaForSearch();
  ...
  CropAreaCircle.prototype.draw=function() {
    cropAreaForSearch.prototype.draw.apply(this, arguments);

    // draw move icon
    var c = this.getCenterPoint();
    this._cropCanvasForSearch.drawIconMove([c.x, c.y], this._areaIsHover?this._iconMoveHoverRatio:this._iconMoveNormalRatio);

    // draw resize cubes
    this._cropCanvasForSearch.drawIconResizeBoxNESW(this._calcResizeIconCenterCoords(), this._boxResizeBaseSize, this._boxResizeIsHover?this._boxResizeHoverRatio:this._boxResizeNormalRatio);
  };
...
cropForSearch.factory('cropAreaRectangleForSearch', ['cropAreaForSearch', function(cropAreaForSearch) {
  ...
  CropAreaRectangle.prototype = new cropAreaForSearch();
  ...
  CropAreaRectangle.prototype.draw=function() {
    ...
    // draw move icon
    this._cropCanvasForSearch.drawIconMove([center.x, center.y], this._areaIsHover?this._iconMoveHoverRatio:this._iconMoveNormalRatio);

    // draw resize thumbs
    var resizeIconsCenterCoords=this._calcRectangleCorners();
    for(var i=0,len=resizeIconsCenterCoords.length;i<len;i++) {
      var resizeIconCenterCoords=resizeIconsCenterCoords[i];
      this._cropCanvasForSearch.drawIconResizeCircle(resizeIconCenterCoords, this._resizeCtrlBaseRadius, this._resizeCtrlIsHover===i?this._resizeCtrlHoverRatio:this._resizeCtrlNormalRatio);
    }
  };
...
cropForSearch.factory('cropAreaSquareForSearch', ['cropAreaForSearch', 'cropAreaRectangleForSearch', function(CropAreacropAreaForSearch, CropAreaRectangle) {
...
cropForSearch.factory('cropAreaForSearch', ['cropCanvasForSearch', function(CropCanvas) {
  var cropAreaForSearch = function(ctx, events) {
    this._ctx=ctx;
    this._events=events;

    this._minSize={x:0, y: 0, w:119, h:100};/*changed*/

    this._cropCanvasForSearch=new CropCanvas(ctx);

    this._image=new Image();
    this._size = {x: 0, y: 0, w:119, h:100};/*changed*/
  };

  /* GETTERS/SETTERS */

  cropAreaForSearch.prototype.getImage = function () {
    return this._image;
  };
  cropAreaForSearch.prototype.setImage = function (image) {
    this._image = image;
  };

  cropAreaForSearch.prototype.getSize = function () {
    return this._size;
  };

  cropAreaForSearch.prototype.setSize = function (size) {

    size = this._processSize(size);
    this._size = this._preventBoundaryCollision(size);
  };

  cropAreaForSearch.prototype.setSizeByCorners = function (northWestCorner, southEastCorner) {
  ...
  cropAreaForSearch.prototype.getSouthEastBound = function () {
    return this._southEastBound(this.getSize());
  };

  cropAreaForSearch.prototype.getMinSize = function () {
    return this._minSize;
  };

  cropAreaForSearch.prototype.getCenterPoint = function () {
    var s = this.getSize();
    return {x: s.x + (s.w / 2),
            y: s.y + (s.h / 2) };
  };

  cropAreaForSearch.prototype.setCenterPoint = function (point) {
    var s = this.getSize();
    this.setSize({x: point.x - s.w / 2, y: point.y - s.h / 2, w: s.w, h: s.h});
  };

  cropAreaForSearch.prototype.setMinSize = function (size) {
    this._minSize = this._processSize(size);
    this.setSize(this._minSize);
  };

  // return a type string
  cropAreaForSearch.prototype.getType = function() {
    //default to circle
    return 'circle';
  }

  /* FUNCTIONS */
  cropAreaForSearch.prototype._preventBoundaryCollision=function(size) {
    ...
    /*new code to restrict aspectRatio (changed)*/
    newSize = {x: newSize.x,
                   y: newSize.y,
                   w: newSize.h * 1.67,
                   h: newSize.h};
               
    if(newSize.w > canvasW){
        newSize = {x: newSize.x,
                   y: newSize.y,
                   w: canvasW,
                   h: canvasW/1.67};
    }
    /*end new code to restrict aspectRatio (changed)*/
  ...
  cropAreaForSearch.prototype._drawArea=function() {};

  cropAreaForSearch.prototype._processSize=function(size)
  {
  ...
  cropAreaForSearch.prototype._southEastBound=function(size)
  {
    return {x: size.x + size.w, y: size.y + size.h};
  }
  cropAreaForSearch.prototype.draw=function() {
    // draw crop area
    this._cropCanvasForSearch.drawCropArea(this._image,this.getCenterPoint(),this._size,this._drawArea);
  };

  cropAreaForSearch.prototype.processMouseMove=function() {};

  cropAreaForSearch.prototype.processMouseDown=function() {};

  cropAreaForSearch.prototype.processMouseUp=function() {};

  return cropAreaForSearch;
}]);


cropForSearch.factory('cropCanvasForSearch', [function() {
...
cropForSearch.factory('cropHostForSearch', ['$document', 'cropAreaCircleForSearch', 'cropAreaSquareForSearch', 'cropAreaRectangleForSearch', function($document, CropAreaCircle, CropAreaSquare, CropAreaRectangle) {
  /* STATIC FUNCTIONS */
    ...
    this.setNewImageSource=function(imageSource) {
      ...
      if(!!imageSource) {
        ...
        newImage.onload = function(){
          /*custom cose to show loader and display image area*/ 
          hideFullLoaderNew();
           $('.croper-profile-for-search').fadeIn('fast');
           /*end custom cose to show loader and display image area*/ 
...
cropForSearch.factory('cropPubSubForSearch', [function() {
  return function() {
    var events = {};
    // Subscribe
...
cropForSearch.directive('imgCropForSearch', ['$timeout', 'cropHostForSearch', 'cropPubSubForSearch', function($timeout, CropHost, CropPubSub) {
  return {
    restrict: 'E',
    scope: {
      image: '=',
    ...
    link: function(scope, element/*, attrs*/) {
      ...
      // Init Crop Host
      var cropHostForSearch=new CropHost(element.find('canvas'), {}, events);

      // Store Result Image to check if it's changed
      var storedResultImage;

      var updateResultImage=function(scope) {
        var resultImageObj=cropHostForSearch.getResultImage();
          ...
          if(angular.isDefined(scope.areaCoords)){
			  ...
			  if(angular.isDefined(resultImageObj.imageSize)){
				  ...
				  scope.originalHeight = resultImageObj.imageSize.h;
                                  ...
                                  /*check validity*/
                                  if(checkdimensionValidity && ((resultImageObj.imageSize.w < minWidthAllowed) || (resultImageObj.imageSize.h < minHeightAllowed))){
                                        dimensionValidFlag = false;
                                       
                                  }else{
                                        dimensionValidFlag = true;
                                       
                                  }
                                  scope.dimensionValidFlag = dimensionValidFlag;
                                  /*end check validity*/
      ...
      var updateAreaCoords=function(scope) {
        var areaCoords=cropHostForSearch.getAreaCoords();
      ...
      scope.$watch('image',function(){
        cropHostForSearch.setNewImageSource(scope.image);
      });
      scope.$watch('areaType',function(){
        cropHostForSearch.setAreaType(scope.areaType);
        updateResultImage(scope);
      });
      scope.$watch('areaMinSize',function(){
        cropHostForSearch.setAreaMinSize(scope.areaMinSize);
        updateResultImage(scope);
      });
      scope.$watch('resultImageSize',function(){
        cropHostForSearch.setResultImageSize(scope.resultImageSize);
        updateResultImage(scope);
      });
      ...
      scope.$watch(
        ...
        function (value) {
          cropHostForSearch.setMaxDimensions(value[0],value[1]);
          updateResultImage(scope);
        },
        true
      );

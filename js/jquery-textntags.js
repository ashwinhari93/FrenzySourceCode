...
(function ($, _, undefined) {
    ...
    var defaultSettings = {
        ...
        templates       : {
            ...
            tagHighlight      : _.template('<strong><span class="user-$<%= userid %>">$<%= idx %></span></strong>'),
    ...
    var TextNTags = function (editor) {
        ...
        beautified_text = beautified_text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
            _.each(settings.triggers, function (trigger) {
                var markup = templates.tagHighlight({idx: trigger.parserGroups.title, userid: trigger.parserGroups.id, class_name: trigger.classes.tagHighlight});
        ...
        $(document).ready(function(){
            $('#composerCtrl button').click(function(){
